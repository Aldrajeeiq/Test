...
async def _(event):
	...
	@sython.on(events.NewMessage(outgoing=True, pattern=r"\.تجميع عملاق"))
async def _(event):
        await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
        channel_entity = await sython.get_entity(bot_username)
        await sython.send_message('@TMWEL10M_BOT', 'جاري التجميع بواسطة | SOMY TEAM')
        channel_entity = await sython.get_entity(bot_username)
        await sython.send_message('@TMWEL10M_BOT', '/start')
        await asyncio.sleep(5)
        msg0 = await sython.get_messages('@TMWEL10M_BOT', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await sython.get_messages('@TMWEL10M_BOT', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await sython(ImportChatInviteRequest(bott))
                msg2 = await sython.get_messages('@TMWEL10M_BOT', limit=1)
                await msg2[0].click(text='التالي')
                chs += 1
